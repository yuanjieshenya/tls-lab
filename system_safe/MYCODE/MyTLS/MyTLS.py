...
import rsa
from MyTLS.TLS_Types import ENCODE_METHOD, myTLSSocketType, messageType
from MyTLS.TLSExceptions import TCPSocketException
from MyTLS.Message import *
from time import time
from random import randint
from MyTLS.Encryption import *
...
class TLSSocket:
    ...
    def __recvPackage(self) -> bytes:
        ...
        length = bytes_to_short(header[2: 4])
        if length == 0:
            return header
...
class TLSClient(TLSSocket):
    ...
    def __diffHandShakes(self) -> None:
        if self._encryMethod == messageType.ENCRY_METHOD_RSA_AES_SHA256:
            ...
            encryptedSessionKey = self._keyMaker.encrypt(int_to_bytes(self._randNum3))
            # 打印出生成的加密会话密钥
            print(f"Generated Encrypted Session Key: {encryptedSessionKey.hex()}")
            self._sendKeyExchangeMessage(key=encryptedSessionKey)
    ...
    def __diffHandShakes(self) -> None:
        if self._encryMethod == messageType.ENCRY_METHOD_RSA_AES_SHA256:
            ...
            if not self._thisCert:
                raise TCPSocketException("未加载证书")
            ...
            print(f"Generated Encrypted Session Key: {clientKeyExchange}")
            self._randNum3 = bytes_to_int(self._keyMaker.decrypt(clientKeyExchange.key))
