from MyTLS.TLSExceptions import WrongMessageException
from MyTLS.type_changer import *
from MyTLS.TLS_Types import *
import rsa
...
class AllMessages:
    ...
    def makeBasicMessage(__type__: int,
        ...
        return bytes([__version__ + __type__, __reserved__]) + short_to_bytes(__length__)
...
class HelloMessage(AllMessages):
    ...
    def __init__(self, msg: bytes):
        ...
        self.time        = bytes_to_int(header[0][0: 4])
        self.randomNum   = bytes_to_int(header[1][0: 4])
        self.encryMethod = bytes_to_int(header[2][0: 4])
    ...
    def makeMessage(__time__: int, __randomNum__: int, __encryMethod__: int) -> bytes:
        ...
        length_4 = short_to_bytes(4)
        msg += length_4 + int_to_bytes(__time__) + \
               length_4 + int_to_bytes(__randomNum__) + \
               length_4 + int_to_bytes(__encryMethod__)
...
class certExchangeMessage(AllMessages):
    ...
    def __init__(self, msg: bytes):
        ...
        self.time      = bytes_to_int(header[2][0: 4])
    ...
    def makeMessage(__publicKey__: rsa.PublicKey, __owner__: str, __time__: int) -> bytes:
        ...
        length_ = short_to_bytes(ttMsg.__len__())
        tMsg = length_ + ttMsg
        ...
        ttMsg = int_to_bytes(__time__)
        length_ = short_to_bytes(4)
        tMsg += length_ + ttMsg
...
class keyExchangeMessage(AllMessages):
    ...
    def makeMessage(__key__: bytes) -> bytes:
        ...
        msg += short_to_bytes(__key__.__len__()) + __key__
        return msg
...
class FinishedMessage(AllMessages):
    ...
    def makeMessage(__finishedMsg__: bytes) -> bytes:
        ...
        return msg + short_to_bytes(__finishedMsg__.__len__()) + __finishedMsg__
